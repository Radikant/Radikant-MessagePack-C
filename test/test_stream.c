...
bool test_stream_memory_read(test_result_t *test) {
    ...
    mp_stream_init_read(&stream, &buffer, data, sizeof(data) - 1); // exclude null terminator
...
bool test_stream_memory_write(test_result_t *test) {
    ...
    if (buffer.size != 11) append_error(test, "Written size mismatch", buffer.size);
    if (memcmp(buffer.data, "hello world", 11) != 0) append_error(test, "Written data mismatch", 0);
