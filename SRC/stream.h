...
typedef mp_error_t (*mp_stream_read_fn)(mp_stream_t *stream, void *buf,
                                        size_t count);
...
// If is_dynamic == true, the stream auto-allocates. buf and capacity are
// ignored. If is_dynamic == false, the stream writes to the fixed buf.
